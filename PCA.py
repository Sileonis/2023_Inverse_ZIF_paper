...
def plot_pca_data(pca_df, title, randomized_order: bool, interpolated: bool, number, trained_model_for_specific_gas, pca_obj, filename = None ):
    ...
    if interpolated:
        ...
        # Transform the grid points back to the original feature space using the PCA inverse_transform function
        grid_points_in_original_space = pca_obj.inverse_transform(np.column_stack((gridX.ravel(), gridY.ravel())))

        # Use the trained model to predict the output property (logD) for the grid points
        predicted_logD = trained_model_for_specific_gas.predict(grid_points_in_original_space)

        # Reshape the predicted_logD to match the shape of gridX and gridY for plotting
        predicted_logD = predicted_logD.reshape(gridX.shape)
        interpZ = predicted_logD
...
# %%
def run_n_plot(order: bool, predict_or_no: bool, step= 20, output_dir="output_data"):
    numbers_list = list(range(step, 111, step))
    OrderRandom = order
    Interpolation = predict_or_no
    ...
    for number in numbers_list:
        ...
        plot_pca_data(pca_df, f'{A}{B} - %{number} of original set', OrderRandom, Interpolation, number, trained_model, pca_obj, filename)

# %%
run_n_plot(True, False, 20, output_dir="output_data")
